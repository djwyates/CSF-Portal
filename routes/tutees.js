...
router.get("/", auth.hasAccessLevel(2), function(req, res) {
  Tutee.find({}, function(err, tutees) {
    if (err) {
      ...
      tutees.sort((a, b) => new Date(b.createdOn) - new Date(a.createdOn));
...
router.post("/", function(req, res) {
  ...
  Tutee.create([newTutee], {new: true}, function(err, tutee) {
    if (err) {
      ...
      Member.findOneAndUpdate({id: tutee[0].id}, {tuteeID: tutee[0]._id}).exec();
      if (req.user && req.user.id == tutee[0].id || req.user && req.user.accessLevel >= 2) {
        req.flash("success", "Your tutoring request has been successfully submitted.");
        res.redirect("/tutees/" + tutee[0]._id + (req.query.from ? "?from=" + req.query.from : ""));
      } else {
        req.flash("success", "Your tutoring request has been successfully submitted. To view your request, login with your email.");
        res.redirect(req.query.from ? req.query.from : "/members/attendance");
      }
    }
  });
});
